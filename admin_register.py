...
def register_user(username, email, password, role="user"):
    ...
    data = {
        ...
        "email": email,
        "password": password,
        "role": role  # Rolü admin ya da user olarak belirtin
    }
    try:
...
if __name__ == "__main__":
    ...
    register_user("superadmin", "superadmin@example.com", "adminpassword", "admin")

    # Admin kullanıcısı ile giriş yapma
    print("Attempting to login admin user...")
    token = login_user("superadmin@example.com", "adminpassword")

    if token:
        print("Admin giriş başarılı! Token:", token)
    else:
